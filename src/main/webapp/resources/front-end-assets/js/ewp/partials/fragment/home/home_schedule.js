 ...
 * 
 */
import {Util, ModalService, AjaxBuilder} from '/resources/core-assets/essential_index.js';
import Semaphore from '/resources/library/async-mutex/es6/Semaphore.js';
import {AssignCall as $AS, UserCall as $USER} from '/resources/front-end-assets/js/ewp/comm/ajax_index.js';
...
const calendarHandler = {
	...
	async setScheduleCard(){
		...
		if(loginType == "SSO" && this.today <= this.selectedDate){
			$assignBtnList.forEach($btn => {
				$btn.style.display = "";
				$btn.onclick = () => {
					location.href = "/ewp/meeting/assign/status/list"+"?holdingDate="+this.selectedDate;
				}
			});
		}
	},
...
const scheduleCardGenerator = {
	...
	createRow(assign){
		...
		const setPropClass = () => {
			...
			const appStatus = assign.appStatus;
			if(appStatus == "APPROVED"){
				propClassSet.add(this.getMeetingStatusProp(assign.meetingStatus));
			}else{
				propClassSet.add(this.getSkdStatusProp(appStatus));
			}
		...
		const createScheduleDiv = () => {
			...
			$roomDiv.innerHTML = room?.officeName + " " + room?.roomName;
		...
		const createHostDiv = () => {
			...
			$hostDiv.innerHTML = assign.skdHost;
			return $hostDiv;
		}
		setPropClass();
...
const evtHandler = {
	...
	setHelpModal(){
		...
		if($btn){
			$btn.onclick = () => {
				ModalService.get("guideForSchedule").show();
			}
		}
	},
	setStatModal(){
		...
		if($btn){
			$btn.onclick = () => {
				ModalService.get("guideForStat").show();
			}
		}
	},
}
			...
			switch(assign.appStatus){
				...
				case "APPROVED":
					...
					if(diffTime < 0) {
						try{
							AjaxBuilder({
								request: $USER.Get.userAuthorityForMeeting,
								param: assign.meetingKey,
								loading: false,
							}).success(authorities => {
...
const dataHandler = {
	...
	async updateDailyStat(momentMonth) {
		return new Promise((resolve, reject) => {
			...
			AjaxBuilder({
				request: $AS.Get.assignStatForPlanned,
	...
	async updateScheduleList(selectedDate){
		console.log("stat", this.dailyStat[selectedDate])
		...
		AjaxBuilder({
			request: $AS.Get.assignListForPlanned,
			param: {
