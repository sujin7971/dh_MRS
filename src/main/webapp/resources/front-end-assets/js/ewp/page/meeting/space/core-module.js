...
import {StorageMng} from './storage-module.js';
import {attendeeHandler} from '/resources/front-end-assets/js/ewp/page/meeting/assign/module_index.js';
import {UserSearchModal} from '/resources/front-end-assets/js/ewp/comm/module_index.js';
import {ScheduleCall as $SKD, AssignCall as $AS, AttendeeCall as $ATT, UserCall as $USER, FileCall as $FILE} from '/resources/front-end-assets/js/ewp/comm/ajax_index.js';
let _timerMng;
let _bookletMng;
...
const scheduleService = {
	setBeginTimer(){
		...
		_timerMng = TimerMng({
			...
			,intervalEndCall: async () => {
				authorities = await $USER.Get.userAuthorityForMeeting(meetingKey);
				initManageBtn();
				this.setFinishTimer();
			}
			,timerFormat : 'ss'
	...
	extSchedule(){
		...
		AjaxBuilder({
			request: $SKD.Get.nextScheduleOne,
			param: skdKey,
			loading: false,
			...
			const $nowScheduleDiv = buildScheduleInfoElem({
				...
				host: instanceProvider.assign.skdHost,
			});
			...
			if(diffMinute < INTERVAL_MINUTE){//더이상 연장을 할 수 없는경우
				...
				if(reply == "OK"){
					...
					AjaxBuilder({
						request: $SKD.Put.scheduleExtend,
						param: {
							skdKey: instanceProvider.skdKey,
							minutes: extRequstMinutes
						},
					}).success(res => {
						SocketService.sendUpdateMsg({
							resourceType: "SCHEDULE",
							data: instanceProvider.assign.skdKey
						});
					}).exe();
				}
			}
...
function sortAttendeeByType(list){
	...
	optList = [];
	attendeeMap = {};
	attendeeList.forEach(attendee => attendeeMap[attendee.attendKey] = attendee);
	for(let i = 0; i < list.length; i++) {
		let row = list[i];
		if(row.attendRole == 'HOST') {
			hostEmp = row;
...
function showAttendeeList() {
	let $con = $("#userList");
	$con.empty();
	$con.append(generateAttendeeDOM(hostEmp, "HOST"));
	if(assistantEmp){
...
async function initManageBtn(){
	...
	//첨부파일 추가
	if(authorities.includes("FUNC_UPLOAD") && loginType == "SSO"){
	...
	if(authorities.includes("FUNC_PHOTO")){
		...
		$cameraBtn.onclick = () => {
			...
			$photoInput.onchange = async (evt) => {
				...
				const uploadRes = await $FILE.Post.meetingFileList({
	...
	if(authorities.includes("FUNC_REPORT")){
		...
		$reportBtn.onclick = () => {
			exitWithSave('/ewp/meeting/'+meetingKey+'/report');
		};
	}
}
...
function generateAttendeeDOM(attendee, attendRole){
	...
	if(attendee){
		...
		$infoBtn.on("click", function(){
			...
			$img.src = "/api/ewp/user/"+attendee.user.userKey+"/img";
			$img.onload = () => {
				Util.removeClass($userPic, "userPic");
				$userPic.appendChild($img);
			}
			$ATT.Get.attendeeOne(attendee).then((result) => {
		...
		if(progress.entrySet.includes(attendee.user.userKey)){
			$name.addClass("blue");
		}
...
function selectNewAssistant(attendee) {
	...
	let assistantYN;
	if(attendee == null){
		attendKey = assistantEmp.attendKey;
		assistantYN = 'N';
	}else{
		attendKey = attendee.attendKey;
		assistantYN = 'Y';
...
async function showAddAttendeeModal(){
	...
	const showUserModal = () => {
		const disabled = attendeeList.map(attendee => attendee.userKey);
...
const connectWebsocket = async () => {
	return new Promise((resolve, reject) => {
		SocketService.init({
			...
			destinationURL: "/update/"+meetingKey,
		});
		SocketService.connected(() => {
			...
			SocketService.subscribe({
				url: '/room/'+meetingKey+'/userKey/'+loginKey,
				receiver: async (e) => {
					const response = JSON.parse(e.body);
		...
		switch(response.resourceType) {
			...
			case 'ATTENDEE' :
				const updateAttendee = async (param)=> {
					...
					let attendee = attendeeMap[param.attendKey];
					if(attendee){
						const {
					...
					attendeeMap[attendee.attendKey] = attendee;
					if(param.assistantYN && attendee.userKey == loginKey){
						authorities = await $USER.Get.userAuthorityForMeeting(meetingKey);
						initManageBtn();
					}
				}
	...
	return new Promise(async (resolve, reject) => {
		...
		switch(response.actionType) {
			...
			case 'LEAVE' :
				...
				if(!progress.entrySet.includes(loginKey)){
					SocketService.sendJoinMsg();
				}
				if(streamer){
...
async function setScreenControlMode(mode){
	...
	switch(mode){
		case "control":{ // 화면 공유
			const streamerData = attendeeList.find(attendee => attendee.user.userKey == streamer);
		...
		switch(response.resourceType) {
			...
			case 'ATTENDEE' :
				for(let attendee of data){
					attendeeMap[attendee.attendKey] = attendee;
					if(attendee.assistantYN == 'Y' && attendee.userKey == loginKey){
						authorities = await $USER.Get.userAuthorityForMeeting(meetingKey);
						initManageBtn();
					}else if(assistantEmp){
						if(attendee.userKey == loginKey && attendee.assistantYN == 'N'){
							authorities = await $USER.Get.userAuthorityForMeeting(meetingKey);
							initManageBtn();
						}
					}
...
async function showSelectedFinishModal(id){
	...
	try{
		...
		switch(res){
			...
			case "FINISH":
				try{
					const res = await $SKD.Put.scheduleFinish({
						skdKey: instanceProvider.assign.skdKey
					});
...
/* 회의 진행자/보조 진행자 여부 */
function isManager(){
	if(loginKey == hostEmp?.userKey || loginKey == assistantEmp?.userKey){
		return true;
	}
	return false;
}
	...
	setExitBtn(){
		...
		if($archiveBtn){
			$archiveBtn.onclick = () => {
				...
				Modal.confirm({msg: msg}).then(async (btn) => {
					if(btn == "OK"){
						exitWithSave("/ewp/meeting/archive/manage/user");
					}
				});
			}
		}
...
;(async () => {
	...
	try{
		...
		const attendeeList = await $ATT.Get.attendeeListByMeeting({
			meetingKey: meetingKey
		});
