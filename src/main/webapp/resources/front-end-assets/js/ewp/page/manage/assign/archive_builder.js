				...
				if(reply == "OK"){
					...
					try{
						...
						for (const [key, values] of Object.entries(delGroup)) {
							const delRes = await $FILE.Delete.meetingFileList({
								meetingKey: key,
								fileList: values.map(file => file.fileKey)
							});
 ...
 * $row.getCheckedFileList() return [{fileKey: 1, originalName: abcd...}, ...] : 선택된 모든 파일 객체리스트 반환
 * 
...
export const archiveRowGenerator = {
	...
	createRow(archive){
		...
		const createNoColumn = () => {
			...
			$column.innerHTML = archive.skdKey;
		...
		const createTitleColumn = () => {
			...
			//$a.href = "/ewp/room/"+archive.roomType+"/assign/"+archive.reqKey;
			...
			$title.onclick = () => {
				archiveEvtHandler.trigger("titleClick", {
					skdKey: archive.skdKey,
				});
			}
			const $host = Util.createElement("div");
			const $span = Util.createElement("span");
			$host.appendChild($span);
			$span.innerHTML = archive.skdHost;
		...
		const createFileColumn = () => {
			...
			const createFileRow = (file) => {
				...
				const [
					...
					downloadable,
					viewable,
				] = typeCollection[file.roleType].state[file.state + 2];//배열0부터 시작이므로 상태값을 2씩 당김
				
				$li.deletable = deletable;
				$li.downloadable = downloadable;
				$li.viewable = viewable;
				
				const $link = Util.createElement("div", "link", typeCollection[file.roleType].cls);
				$li.appendChild($link);
