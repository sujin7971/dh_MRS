...
import {MeetingCall as $MEETING, RoomCall as $RM} from '/resources/front-end-assets/js/ewp/ajax/ajax_index.js';
...
const domHandler = {
	...
	async setUpdateMode(){
		const assign = await $MEETING.Get.assignOne(skdKey);
		if(assign){
...
const requestHandler = {
	validation(){
		...
		const skdHost = formHelper.getForm("skdHost").getValue();
		if(Util.isEmpty(skdHost)){
			throw {
				status: 404,
				message: "필수 입력 누락",
				detail: "주최자를 입력해 주세요."
	...
	async insertMeeting(){
		...
		try{
			...
			const skdKey = assignResult.data.skdKey;
	...
	async updateMeeting(){
		...
		try{
			this.validation();
			const skdKey = formHelper.getForm("skdKey").getValue();
			const meetingId = formHelper.getForm("meetingId").getValue();
			const assignResult = await this.updateAssign(skdKey);
			const elecYN = formHelper.getForm("elecYN").getValue();
			if(elecYN == 'Y'){
				await this.updateAttendee(meetingId);
				await this.updateFile(meetingId);
	...
	async updateAssign(skdKey){
		try{
			const formData = formHelper.getFormData();
			const result = await $MEETING.Put.assignOne({skdKey: skdKey, formData: formData});
			if(result.status != 200){
				throw result;
			}
			return result;
...
const evtHandler = {
	...
	enableScheduleHostClearBtn(){
		...
		if($btn){
			$btn.onclick = () => {
				formHelper.getForm("skdHost").setValue("");
			}
		}
	},
