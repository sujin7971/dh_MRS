...
import {FileCall as $FILE, NoteCall as $NOTE} from '/resources/front-end-assets/js/ewp/comm/ajax_index.js';
/**
...
export const BookletMng = function(instanceProvider) {
	const loginKey = instanceProvider.loginKey;
	const meetingKey = instanceProvider.meetingKey;
	const FileService = instanceProvider.getFileService();
	const mutex = new Mutex();
	const dbConnector = new IDBConnector({DB_NAME: loginKey+"_"+meetingKey+"_DRAWING_ARCHIVE"});
	let undoStack = [];
	let redoStack = [];
	let pagechecksum = "";
	...
	async function saveContextOnDB(graphicContext){
		...
		await dbConnector.insert(doc["fileKey"], graphicContext);
		redoStack = [];
	...
	async function loadPage(data = {}){
		return new Promise(async (resolve, reject) => {
			...
			if(fileChange || pageChange){
				...
				instance.trigger("selectFile", file.fileKey, pageno);
				if(doc["pageno"] && save){
				...
				if(fileChange){
					if(file == "MEMO"){
						...
						await setBookmarkThumbList(file.fileKey);
				...
				if(pageChange){
					...
					if(file == "MEMO"){
						...
						await setFileThumbList(file.fileKey, pageno);
					}
					setTimeout(() => {
		...
		async function selectFile(file, pageno){
			return new Promise(async (resolve, reject) => {
				try{
					doc["file"] = file;
					doc["fileKey"] = file.fileKey;
					await dbConnector.setStore(file.fileKey);
					await loadFileGraphicContext();
					const pageLength = file.page;
		...
		async function selectMemo(){
			return new Promise(async (resolve, reject) => {
				try{
					...
					await loadFileGraphicContext();
					const pageLength = await $NOTE.Get.memoPageLength(meetingKey);
					doc["pageLength"] = pageLength
					initPaginationCtrl();
		...
		function selectPage(pageno){
			return new Promise(async function(resolve, reject) {
				...
				if(fileKey == "MEMO"){
					src = $NOTE.Get.memoSourceImage({
						meetingKey: meetingKey,
					});
				}else{
					src = $NOTE.Get.notePageSourceImage({
						meetingKey: meetingKey,
						fileKey: fileKey,
						pageNo: pageno
					});
	...
	async function loadFileGraphicContext(){
		const context = await dbConnector.getAllData(doc["fileKey"]);
		console.log("loadFileGraphicContext", "fileKey", doc["fileKey"], "GraphicContext", context);
		if(context){
			undoStack = context;
			redoStack = [];
		}
	...
	function bindPaginationEvt(){
		...
		async function pageLoadAndScroll(pageno){
			...
			if(fileKey == "MEMO"){
				...
				await setFileThumbList(fileKey, pageno);
			}
			try{
	...
	async function loadPageGraphicContext(){
		...
		const pageGraphicContextList = await dbConnector.getDataFromIndex(doc["fileKey"], "page", pageno);
	...
	async function setBookmarkThumbList(fileKey){
		const bookmarkList = await dbConnector.getBookmark(fileKey);
		...
		for(let i = 0; i < bookmarkList.length; i++){
			...
			let $bookCon = (fileKey == "MEMO")?generateMemoThumbDOM({pageno: pageno, includeOption: false}):generateFileThumbDOM(fileKey, pageno);
	...
	async function setFileThumbList(fileKey, pageno){
		unbindThumbScrollEvt();
		//console.log("setFileThumbList", "fileKey", fileKey, "pageno", pageno);
		openThumbnail();
		...
		while(true){
			...
			await setFileThumbInRange(fileKey, startPage, endPage);
			...
			if(!isScrollbarVisible){
				...
				await setFileThumbList(fileKey, pageno + 10);
			}
		}
		bindThumbScrollEvt();
	}
	...
	/* 페이지 범위내 섬네일 생성 */
	function setFileThumbInRange(fileKey, startPage, endPage){
		//console.log("setFileThumbInRange", "fileKey", fileKey, "startPage", startPage, "endPage", endPage);
		return new Promise(async (resolve, reject) => {
			const bookmark = await dbConnector.getBookmark(fileKey);
			doc["bmPageList"] = bookmark;
			const $thumblist = [];
			for(let i = startPage; i <= endPage; i++){
				let pageno = i;
				let $thumbCon = generateFileThumbDOM(fileKey, pageno);
	...
	/* 썸네일 DOM 생성 */
	function generateFileThumbDOM(fileKey, pageno){
		let html = thumbHTML;
		let $imgCon = $(html.imgCon);
		let $img = $(html.img);
		$imgCon.html($img);
		$imgCon.attr("data-page",pageno);
		const src = $NOTE.Get.notePageDrawImage({
			meetingKey: meetingKey,
			fileKey: fileKey,
			pageNo: pageno,
			salt: pagechecksum,
	...
	function generateMemoThumbDOM(data = {}){
		...
		$imgCon.html($img);
		$imgCon.attr("data-page",pageno);
		const src = $NOTE.Get.memoPageDrawImage({
			meetingKey: meetingKey,
			pageNo: pageno,
	...
	async function addMemoPage(pageno){
		...
		await $NOTE.Post.memoPage({
			meetingKey: meetingKey,
			pageNo: pageno,
		});
		await dbConnector.shiftRightIndex(doc["fileKey"], "page", pageno);
	...
	async function deleteMemoPage(pageno){
		...
		await $FILE.Delete.memoPage({
			meetingKey: meetingKey,
			pageNo: pageno,
		});
		await dbConnector.deleteDataFromIndex(doc["fileKey"], "page", pageno);
		await dbConnector.shiftLeftIndex(doc["fileKey"], "page", pageno);
		doc["pageLength"] = newLength;
		doc["file"] = null;
	...
	function undo(){
		return new Promise(async (resolve, reject) => {
			...
			await dbConnector.deleteLast(doc["fileKey"]);
			limePad.undo();
			setUndoRedoState();
			resolve();
		});
	...
	function redo(){
		return new Promise(async (resolve, reject) => {
			...
			await dbConnector.insert(doc["fileKey"], context);
		...
		$(".thumbnailDiv").on("scroll", async function() {
	        ...
	        if (scrollTop + innerHeight >= scrollHeight - 10) {
	        	...
	        	if(fileKey == "MEMO"){
					...
					await setFileThumbList(fileKey, startPage);
				}
				bindThumbScrollEvt();
	        }
		});
	...
	function bookmarkPage(pageno){
		...
		for(let i = 0; i < bookmark.length; i++){
			let $imgCon = (fileKey == "MEMO")?generateMemoThumbDOM({pageno: bookmark[i], includeOption: false}):generateFileThumbDOM(fileKey, bookmark[i]);
		...
		doc["bmPageList"] = bookmark;
		dbConnector.setBookmark(fileKey, bookmark);
	}
	/* 북마크 제거 */
	...
	function rightShiftBookmark(addpage){
		...
		dbConnector.setBookmark(fileKey, shiftedBookmark);
	}
	...
	function removePageImage(){
		return new Promise(async (resolve, reject) => {
			...
			try{
				if(fileKey == "MEMO"){
					await $NOTE.Put.memoPage({
						meetingKey: meetingKey,
						pageNo: pageno,
					});
				}else{
					await $NOTE.Put.notePage({
						meetingKey: meetingKey,
						fileKey: fileKey,
						pageNo: pageno,
					});
