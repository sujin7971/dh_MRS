...
const getAteendeeClassList = () => {
	const $hostBtnList = document.querySelectorAll("button[data-btn=hostBtn]");
	const $hostDiv = document.querySelector("#hostDiv");
	...
	return [
		{role: "HOST", selectbox: "radio", btnList: $hostBtnList, nodeDiv: $hostDiv}, 
...
export default {
	...
	// 회의 진행자 선택 여부 확인
	validation(){
		const host = this.newList.filter(user => user.attendRole == "HOST");
		...
		const getFilteredList = (list, filter) => {
			...
			list.forEach(user => {
				if(user.attendRole == filter){
					selected.push(user.userKey);
				}else{
					disabled.push(user.userKey);
				}
			});
			return {
				selected: selected,
	...
	setAttendeeList(attendeeList){
		attendeeList.forEach(node => {
			if(node.attendRole == "HOST"){
				this.deleteRole("HOST");
			}
	...
	displayAttendeeList(){
		...
		getAteendeeClassList().forEach(cls => {
			if(cls.nodeDiv){
				classMap[cls.role] = {
						nodeDiv: cls.nodeDiv,
						isDeletable: (cls.role == "HOST")?false:true
				}
		...
		const createUserElem = (attendee, isDeletable) => {
			console.log("createUserElem", attendee)
			...
			if(this.editable && isDeletable){
				...
				$delBtn.onclick = () => {
					const size = this.deleteUser(attendee.userKey);
	...
	getChangeResult(){
		...
		this.newList.forEach(newData => {
			...
			for(const [index, oldData] of this.oldList.entries()){
				if(oldData.userKey == newData.userKey){
