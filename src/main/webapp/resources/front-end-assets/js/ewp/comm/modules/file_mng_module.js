...
import {Final} from '/resources/front-end-assets/js/ewp/comm/module_index.js';
...
export default {
	...
	setBtnEvt(){
		...
		const setDownBtnEvt = ($btn) => {
			this.validDownBtn = () => {
				const bool = this.isSelectedDowloadable();
				$btn.disabled = (!bool);
			}
			$btn.onclick = () => {
				const selectedFileList = this.getCheckedFileList();
				$FILE.Get.downloadMeetingFileList(selectedFileList.map(file => file.fileKey));
			}
		}
		...
		// 파일 다운로드 이벤트 등록
		const $fileDownBtn = Util.getElement("#fileDownBtn");
		if($fileDownBtn){
			$fileDownBtn.disabled = true;
			setDownBtnEvt($fileDownBtn);
		}
		// 파일 삭제 이벤트 등록
	...
	setFileBox(files){
		...
		const editableElemGenerator = (file) => {
			...
			if(this.deletable == true){
				...
				$del.onclick = async () => {
					...
					$li.remove();
					const fileKey = (file.fileKey)?file.fileKey:file.fileId;
					this.deleteFiles(fileKey);
		...
		const viewElemGenerator = (file) => {
			...
			const $typeTd = Util.createElement("td", "text-align-center", typeCollection[file.roleType].cls);
			...
			$nameSpan.innerHTML = file.originalName;
			$nameDiv.appendChild($nameSpan);
			
			if($tr.viewable == true){
				Util.addClass($nameTd, "cursor-pointer");
				$nameTd.onclick = () =>{
					$FILE.Get.viewMeetingFileOne({
						mime: file.mimeType,
						link: file.fileKey
					});
				}
	...
	deleteFiles(...keys){
		this.newList = this.newList.filter(file => !keys.includes(file.fileKey));
		this.setLimitInfo();
	},
	clearFiles(){
		this.newList = [];
	...
	// 추가된 파일 목록 요청
	getAddedFiles(){
		const oldListKeys = this.oldList.map(file => file.fileKey);
		return this.newList.filter(file => !oldListKeys.includes(file.fileKey));
	},
	// 삭제된 파일키 목록 요청
	getDeletedFiles(){
		const newListKeys = this.newList.map(file => file.fileKey);
		return this.oldList.filter(file => !newListKeys.includes(file.fileKey)).map(file => file.fileKey);
	},
}