 ...
 * 5. roomKey: 장소 선택. 초기화 함수 initRoom 지원
...
export default {
	...
	initRoomSelect(){// 장소 선택
		...
		const selectOption = (value) => {
			...
			this.trigger("change", "roomKey", value);
		}
		...
		this.setRoomSelectBox = (roomList) => {
			...
			for(const room of roomList){
				const $roomOption = createOption(room.roomKey, room.roomName);
				$roomSelect.appendChild($roomOption);
			}
		}
		const $allOption = createOption(null, "전체");
		$roomSelect.appendChild($allOption);
		this.selectRoom(this.roomKey);
	},
