...
const requestHandler = {
	...
	async getDeptUserList(officeCode, deptId){
		const list = await $ORG.Get.userList({
...
const userDataHandler = {
	...
	deleteUserList(keyList){
		const userBook = this.getUserBook();
		keyList.forEach(userId => {
			delete userBook[userId];
		});
	},
	getUserByKey(userId){
		const userBook = this.getUserBook();
		return userBook[userId];
	}
}
const modalHandler = {
	init(options = {}){
		...
		const createModalContent = () => {
			...
			const deselectUser = (key) => {
				...
				this.selectedList.forEach(user => {
					if(!key || user.userId == key){
						const $checkbox = $searchResultDiv.querySelector("#user"+user.userId);
						if($checkbox){
							$checkbox.checked = false;
						}
						const $attend = $attendListDiv.querySelector("#attend"+user.userId);
						if($attend){
							$attend.remove();
						}
					}else{
			...
			//선택된 사용자 표시
			const selectUser = (userId, nameplate) => {
				const $attend = Util.createElement("div", "attend");
				$attend.id = "attend"+userId;
			...
			//검색된 사용자 체크버튼과 함께 표시
			const showUser = (deptId, userId, nameplate) => {
				...
				//선택불가한 노드 비활성화
				if(this.disabled.includes(userId)){
					$checkbox.disabled = true;
					Util.addClass($li, "disabled");
				}
				//현 모달에서 선택한 노드 표시
				const selectedKeyList = this.selectedList.map(data => data.userId);
				if(selectedKeyList.includes(userId)){
					$checkbox.checked = true;
				}
				$checkbox.onchange = () => {
			...
			tabList.forEach((tab, index) => {
				tab.on("search", async (param) => {
					...
					const userList = await $ORG.Get.userList({
					...
					if(!Util.isEmpty(userList)){
						...
						userList.forEach(user => {
							showUser(user.deptId, user.userId, (nameplate)?user.nameplate:user.userName);
						});
						this.setAllUserCnt();
						this.setSelectedUserCnt();
					}
...
export default {
	...
	getUser(userId){
		return userDataHandler.getUserByKey(userId);
	}
}