...
const inputHandler = {
	...
	setConfirmCount(attendeeList){
		...
		const confirmCnt = attendeeList.filter(attendee => attendee.attendYN == 'Y').length;
...
const putReport = async (data) => {
	Modal.startLoading();
	const res = await $MEETING.Put.report(data);
	Modal.endLoading();
	return res;
}
...
const evtHandler = {
	init(data){
		...
		this.setModBtn(meetingId);
		this.setCloseBtn(scheduleId);
		this.setCancelBtn(meetingId);
		this.setTempSaveBtn(meetingId);
		this.setOpenOpnBtn(meetingId);
		this.setFinishBtn(meetingId);
	},
	setModBtn(meetingId){
		const $btn = Util.getElement("#modBtn");
		if($btn){
			$btn.onclick = () => {
				location.href = `/lime/meeting/${meetingId}/report/post`;
			}
		}
	},
	setCloseBtn(scheduleId){
	...
	setFinishBtn(meetingId){
		...
		if($btn){
			$btn.onclick = async () => {
				...
				$btn.disabled = true;
				const reportContents = editorHandler.get();
				const reportFile = await editorHandler.saveAsPDF();
				AjaxBuilder({
					request: $MEETING.Put.report,
					param: {
						meetingId: meetingId,
...
export default {
	...
	async setReport(){
		try{
			const assign = await $MEETING.Get.assignOne(this.scheduleId);
			if(assign){
			...
			const report = await $MEETING.Get.reportOne(this.meetingId);
			if(report){
	...
	async setAttendee(){
		try{
			const attendeeList = await $MEETING.Get.attendeeListByMeeting({meetingId: this.meetingId});
