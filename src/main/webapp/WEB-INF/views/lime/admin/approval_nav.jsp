...
<c:set var = "userDetails" value = "${sessionScope.SPRING_SECURITY_CONTEXT.authentication.details}"/>
 ...
 <c:if test='${userDetails.hasPosition("MNG_ITEM_MR")}'>
   <div class="subTab approvalTab" data-room="MEETING_ROOM" onclick="location.href='/lime/manager/approval/manage/meeting?roomType=MEETING_ROOM'">회의실<br>사용신청목록 관리</div>
</c:if>
<c:if test='${userDetails.hasPosition("MNG_ITEM_ER")}'>
   <div class="subTab approvalTab" data-room="EDU_ROOM" onclick="location.href='/lime/manager/approval/manage/meeting?roomType=EDU_ROOM'">강의실<br>사용신청목록 관리</div>
</c:if>
<c:if test='${userDetails.hasPosition("MNG_ITEM_HALL")}'>
   <div class="subTab approvalTab" data-room="HALL" onclick="location.href='/lime/manager/approval/manage/meeting?roomType=HALL'">강당<br>사용신청목록 관리</div>
</c:if>
