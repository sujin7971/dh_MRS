...
function showSelectAssistantModal(list, callback){
	...
	const listMap = list.reduce((acc, attendee) => {
		acc[attendee.attendId] = attendee;
		return acc;
	}, {});
	...
	$selectAssistant.find(".btn-blue").on("click", function(){
		let attendId = $("input[name='assistantRadio']:checked").val();
		callback(listMap[attendId]);
...
function makeSelectAssistantTemplate(attendee) {
	...
	if(attendee != null){
		$input.val(attendee.attendId);
		$input.attr("id", "att"+attendee.attendId);
		$input.attr("name", "assistantRadio");
		$label.html(attendee.user.nameplate);
		$label.attr("for", "att"+attendee.attendId);
	}else{
		$input.val(0);
		$input.attr("id", "att0");
		$input.attr("name", "assistantRadio");
