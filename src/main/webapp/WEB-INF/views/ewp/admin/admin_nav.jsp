...
<c:if test='${userDetails.hasRole("ROLE_SYSTEM_ADMIN") or userDetails.hasRole("ROLE_MASTER_ADMIN")}'>
   <c:if test='${userDetails.hasRole("ROLE_MASTER_ADMIN")}'>
      <div class="subTab archiveTab" onclick="location.href='/ewp/admin/master/manage/meeting/archive'">파일함 관리</div>
   </c:if>
   <div class="subTab noticeTab" onclick="location.href='/ewp/admin/system/manage/notice'">공지사항 관리</div>
   <div class="subTab roomTab" onclick="location.href='/ewp/admin/system/manage/room'">장소 관리</div>
</c:if>
<c:if test='${userDetails.hasRole("ROLE_DEV")}'>
   <div class="subTab sessionTab" onclick="location.href='/ewp/dev/manage/session'">세션 접속 목록</div>
</c:if>
