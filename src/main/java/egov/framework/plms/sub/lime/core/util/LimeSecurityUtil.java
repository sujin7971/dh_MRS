...
/**
 * LimeSecurityUtil 클래스는 스프링 시큐리티와 관련된 유틸리티 메소드를 제공합니다.<br>
 *
...
public class LimeSecurityUtil {
    ...
    /**
     * 현재 인증된 사용자의 LimeAuthenticationDetails 객체를 반환합니다.
     * 인증되지 않은 사용자의 경우 null을 반환합니다.
     * 
     * @return LimeAuthenticationDetails 객체 또는 null
     */
    public static LimeAuthenticationDetails getAuthenticationDetails() {
        if (isAnonymous()) {
            return null;
        }

        Authentication authentication = getAuthentication();
        return (LimeAuthenticationDetails) authentication.getDetails();
    }
    
    public static boolean hasRole(DomainRole domainRole) {
    	LimeAuthenticationDetails details = getAuthenticationDetails();
    	if(details == null) {
    		return false;
    	}else {
